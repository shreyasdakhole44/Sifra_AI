...
Unsafe ladder placement without safety harness lanyard anchor during height inspection.`);
  ...
  const handleSingleReportSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (hasFiles) {
        finalReport = await reportsApi.uploadAttachments(created.id, evidenceFiles);
      }

      setSubmittedReportResult(finalReport);
      setReportText('');
      setEvidenceFiles([]);
      loadDashboardData();
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex space-x-1 border-b border-slate-200 overflow-x-auto">
        ...
        <button
          ...
          className={`px-4 py-2 text-xs font-semibold rounded-t-lg transition-all border-b-2 flex items-center space-x-2 whitespace-nowrap ${
            activeTab === 'moderation' || activeTab === 'reports'
              ? 'border-teal-700 text-teal-700 bg-teal-50/50'
              : 'border-transparent text-slate-600 hover:text-slate-900 hover:bg-slate-50'
          }`}
        >
          <span>Reports & AI Analysis</span>
          {unassignedCount > 0 && (
      ...
      {activeTab === 'overview' && (
        <div id="incident-report-form" className="space-y-4">
          {!submittedReportResult ? (
            ...
            /* AFTER SUCCESSFUL SUBMISSION: SUBMITTED INCIDENT AI ANALYSIS / FINAL TRUST REPORT */
              ...
              <div className="p-3 bg-white border border-slate-200 rounded-lg flex items-center justify-between shadow-xs">
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-xs font-bold text-teal-800 bg-teal-50 px-2.5 py-1 rounded border border-teal-200">
                    AI Safety Analysis & Evidence Panel &bull; Submitted Incident Result
                  </span>
                </div>
              </div>

              <TrustReportView report={submittedReportResult} />
            </div>
          )}
        </div>
      )}
      ...
      {/* TAB 2: MODERATION QUEUE / REPORTS */}
      {(activeTab === 'moderation' || activeTab === 'reports') && (
        <div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-hidden">
      ...
      {/* TAB 4: SAFETY QUIZZES MANAGEMENT */}
      {activeTab === 'quizzes' && (
        <div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-hidden">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-slate-900">Safety Quizzes & Compliance Training Assignments</h3>
              <p className="text-xs text-slate-500">Auto-generated RAG quizzes dispatched to workers upon incident filing.</p>
            </div>
          ...
          <div className="p-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                <span className="text-xs font-semibold text-slate-500 block">Total Quizzes Generated</span>
                <span className="text-xl font-bold font-mono text-slate-900 mt-1 block">{reports.length}</span>
              </div>
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                <span className="text-xs font-semibold text-slate-500 block">Active Worker Assignments</span>
                <span className="text-xl font-bold font-mono text-teal-700 mt-1 block">{reports.length * 2}</span>
              </div>
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                <span className="text-xs font-semibold text-slate-500 block">Average Worker Score</span>
                <span className="text-xl font-bold font-mono text-emerald-600 mt-1 block">85%</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* TAB 5: PDF TRUST REPORTS ARCHIVE */}
      {activeTab === 'pdf-reports' && (
        <div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-hidden">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-slate-900">ReportLab PDF Trust Reports Archive</h3>
              <p className="text-xs text-slate-500">Download formatted PDF Trust Reports synthesized by the backend ReportLab engine.</p>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-slate-200">
                {reports.map((rep) => (
                  <tr key={rep.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-3 font-mono text-slate-700">{rep.worker_id || 'OIL-W-101'}</td>
                    <td className="p-3 font-bold text-slate-900">{rep.risk_level}</td>
                    <td className="p-3 text-slate-500 font-mono">{new Date(rep.timestamp).toLocaleDateString()}</td>
                    <td className="p-3 text-right">
                      <button
                        onClick={() => handleDownloadPdf(rep.id)}
                        className="px-3 py-1 bg-teal-700 text-white rounded text-xs font-semibold hover:bg-teal-800 transition-colors inline-flex items-center space-x-1"
                      >
      ...
      {activeTab === 'settings' && (
        <div className="bg-white rounded-lg border border-slate-200 shadow-xs p-6 space-y-6">
          <div>
            <h3 className="text-sm font-bold text-slate-900">HSC Officer System Preferences</h3>
            <p className="text-xs text-slate-500 mt-1">Configure notification channels, risk threshold triggers, and station assignments.</p>
          </div>

          <div className="space-y-4 max-w-xl">
            <div className="p-4 bg-slate-50 rounded-lg border border-slate-200 space-y-2">
              <span className="text-xs font-bold text-slate-800">SMS Warning Integration (Twilio)</span>
              <p className="text-xs text-slate-500">Automated dispatch enabled for SIF risk &gt; 50%.</p>
            </div>
            <div className="p-4 bg-slate-50 rounded-lg border border-slate-200 space-y-2">
              <span className="text-xs font-bold text-slate-800">Email Report Dispatch (SendGrid)</span>
              <p className="text-xs text-slate-500">ReportLab PDFs attached automatically to supervisor dispatches.</p>
            </div>
          </div>
        </div>
      )}
